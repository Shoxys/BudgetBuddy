...
/**
 * Repository interface for managing {@link Transaction} entities, providing CRUD operations and
 * custom queries for retrieving transactions, summaries, and expense analyses by user, account, and
 * date ranges.
 */
@Repository
public interface TransactionRepo extends JpaRepository<Transaction, Long> {
   ...
   */
  Page<Transaction> findByUser_IdOrderByDateAsc(
   ...
   * @return a page of transactions
   */
  Page<Transaction> findByUser_IdOrderByDateDesc(
      @Param("userId") Long userId, @Param("pageable") Pageable pageable);
  ...
  @Query(
      value =
        ...
        """,
      nativeQuery = true)
  BigDecimal getTotalCreditBetween(
  ...
  @Query(
      value =
        ...
        """,
      nativeQuery = true)
  BigDecimal getTotalDebitBetween(
      @Param("userId") Long userId,
      @Param("startDate") LocalDate startDate,
      @Param("endDate") LocalDate endDate);
   ...
   */
  @Query(
      "SELECT MONTH(t.date) as month, SUM(t.amount) as income "
          + "FROM Transaction t "
