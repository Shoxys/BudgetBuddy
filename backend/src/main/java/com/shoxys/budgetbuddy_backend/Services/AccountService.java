...
public class AccountService {
   ...
   */
  public AccountService(AccountRepo accountRepo, UserRepo userRepo, TransactionRepo transactionRepo,
  ...
  public List<Account> getAccountsByUserId(Long userId) {
    ...
    logger.info("Retrieved {} accounts for user ID: {}", accounts == null ? 0 : accounts.size(), userId);
   ...
   */
  @Transactional
  public Account upsertAccountBalance(String email, Long id, String name, AccountType type, BigDecimal newBalance) {
    logger.debug("Upserting account balance for user: {}, ID: {}, name: {}, type: {}", email, id, name, type);
    ...
    Optional<Account> optionalAccount = (id != null)
  ...
  public Account createSpendingAccount(User user, BigDecimal balance) {
    ...
    logger.info("Created spending account ID: {} for user ID: {}", savedAccount.getId(), user.getId());
    return savedAccount;
  }
  ...
  public Account createGoalSavingsAccount(User user, BigDecimal balance) {
    logger.debug("Creating goal savings account for user ID: {}", user != null ? user.getId() : null);
    if (user == null) {
      logger.error("User is null");
      throw new IllegalArgumentException("User cannot be null");
    }
    ...
    Account savedAccount = accountRepo.save(newAccount);
    logger.info("Created goal savings account ID: {} for user ID: {}", savedAccount.getId(), user.getId());
    return savedAccount;
  }
  ...
  public Account syncSpendingAccountBalance(Transaction transaction, BigDecimal newAmount) {
    logger.debug("Syncing spending account balance for transaction ID: {}", transaction != null ? transaction.getId() : null);
    if (transaction == null) {
    ...
    Account savedAccount = accountRepo.save(account);
    logger.info("Synced account ID: {} balance to: {}", savedAccount.getId(), savedAccount.getBalance());
    return savedAccount;
  }
  ...
  public void updateAccountBalance(Account account, BigDecimal newBalance) {
    logger.debug("Updating account balance for account ID: {}", account != null ? account.getId() : null);
    if (account == null) {
  ...
  public BigDecimal getAccountBalance(String email, String name, AccountType type) {
    ...
    BigDecimal balance = accountRepo.findByUserAndNameAndType(user, name, type)
            .map(Account::getBalance)
            .orElse(null);
    logger.info("Retrieved balance: {} for user: {}, name: {}, type: {}", balance, email, name, type);
    return balance;
  }
  ...
  public void recalculateBalanceForSpendingAccount(Account account) {
    logger.debug("Recalculating balance for spending account ID: {}", account != null ? account.getId() : null);
    if (account == null) {
      logger.error("Account is null");
      throw new IllegalArgumentException("Account cannot be null");
    }
    Utils.validatePositiveId(account.getId(), "Account ID must be positive");
    transactionRepo.findByAccountOrderByDateDesc(account)
            .stream()
            .findFirst()
            .ifPresent(transaction -> {
              BigDecimal balance = transaction.getBalanceAtTransaction();
              if (Utils.nullOrNegative(balance)) {
                logger.error("Invalid balanceAtTransaction: {} for transaction ID: {}", balance, transaction.getId());
                throw new IllegalArgumentException("Transaction balance cannot be null or negative");
              }
              account.setBalance(balance);
              accountRepo.save(account);
              logger.info("Recalculated balance to: {} for account ID: {}", balance, account.getId());
            });
  ...
  public void recalculateGoalSavingsBalance(User user) {
    logger.debug("Recalculating goal savings balance for user ID: {}", user != null ? user.getId() : null);
    if (user == null) {
      logger.error("User is null");
      throw new IllegalArgumentException("User cannot be null");
    }
    ...
    BigDecimal totalContributed = Optional.ofNullable(savingGoalsRepo.sumContributionsByUser(user))
    ...
    logger.info("Recalculated goal savings balance to: {} for user ID: {}", totalContributed, user.getId());
  }
  ...
  public Account handleFetchAccount(User user) {
    logger.debug("Fetching or creating spending account for user ID: {}", user != null ? user.getId() : null);
    if (user == null) {
      logger.error("User is null");
      throw new IllegalArgumentException("User cannot be null");
    }
    ...
    Account account = accountRepo.findAccountByUserAndType(user, AccountType.SPENDING)
            .orElseGet(() -> {
              logger.debug("No spending account found, creating new for user ID: {}", user.getId());
              return createSpendingAccount(user, BigDecimal.ZERO);
            });
    logger.info("Fetched or created spending account ID: {} for user ID: {}", account.getId(), user.getId());
    return account;
  }
}
