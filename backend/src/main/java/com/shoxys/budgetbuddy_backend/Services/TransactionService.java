...
public class TransactionService {
  ...
  public TransactionSummaryResponse getTransactionSummaryByTimeFrame(
    ...
    LocalDate earliest =
        transactions.stream().map(Transaction::getDate).min(LocalDate::compareTo).orElse(null);

    LocalDate latest =
        transactions.stream().map(Transaction::getDate).max(LocalDate::compareTo).orElse(null);
  ...
  public BigDecimal getCurrentBalanceByUser(String email) {
    User user = userRepo.getUserByEmail(email).orElseThrow(() -> new UserNotFoundException(email));

    // Find associated spending account else create new account with initial balance as 0
    Account spendingAccount =
        accountRepo
            .findAccountByUserAndType(user, SPENDING)
  ...
  public Transaction addTransaction(String email, TransactionRequest request) {
    ...
    Account spendingAccount =
        accountRepo
            ...
            .orElseGet(
                () ->
                    accountService.createSpendingAccount(user, request.getBalanceAtTransaction()));

    TransactionType type = request.getType();
    // Change to negative values if type DEBIT
    BigDecimal amount =
        type == DEBIT ? request.getAmount().multiply(BigDecimal.valueOf(-1)) : request.getAmount();
  ...
  public List<Transaction> importMultipleCSVs(long userId, MultipartFile file) {
    ...
    try (BufferedReader reader = new BufferedReader(new InputStreamReader(file.getInputStream()))) {
      ...
      for (CSVRecord record : csvParser) {
        ...
        String merchant =
            record.isSet(MERCHANT.getStr()) && !record.get(MERCHANT.getStr()).isEmpty()
