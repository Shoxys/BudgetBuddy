...
class DashboardServiceTest {
    ...
    void getAccountSummary_shouldMapCorrectly() {
        ...
        when(accountRepo.findAccountsTypeNameBalanceByUser_Id(userId)).thenReturn(Collections.singletonList(mockAccount));
    ...
    void getNetworthResponse_shouldMapCorrectly() {
        when(accountRepo.findAccountsNameBalanceByUser_Id(userId)).thenReturn(Collections.singletonList(mockAccount));
        when(accountRepo.findTotalBalanceByUserId(userId)).thenReturn(totalBalance);
    ...
    void getSavingGoalSummary_shouldMapCorrectly() {
        when(savingGoalsRepo.findTop3ByUser_IdOrderByTargetDesc(userId)).thenReturn(Collections.singletonList(mockGoal));
    ...
    void getRecentTransactions_shouldReturnExpectedList() {
        ...
        when(transactionRepo.findLatest3TransactionSummaries(userId)).thenReturn(Collections.singletonList(txn));
    ...
    void getSpendingInsights_shouldReturnPredefinedInsights() {
        ...
        assertEquals("Consider shifting just 1 meal/week to home cooking, you could save $120/month while still enjoying the occasional treat!", insights.get(0).getInsight());
        assertEquals("Bundling or pausing just 1–2 rarely used ones could save around $25–$40/month, with no major impact on your routine!", insights.get(1).getInsight());
    }
