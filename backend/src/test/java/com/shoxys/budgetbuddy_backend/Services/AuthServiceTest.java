...
public class AuthServiceTest {
    ...
    static class TestData {
        ...
        static final String HASHED_PASSWORD = Password.hash(VALID_PASSWORD).addRandomSalt().withScrypt().getResult();
        ...
        static final RegisterRequest REGISTER_REQUEST = new RegisterRequest(VALID_EMAIL, VALID_PASSWORD);
    }

    @Test
    void authenticate_shouldReturnAuthResponse() {
        // Arrange
        var token = new UsernamePasswordAuthenticationToken(TestData.VALID_EMAIL, TestData.VALID_PASSWORD);
    ...
    void authenticate_shouldThrowIfUserNotFound() {
        var token = new UsernamePasswordAuthenticationToken(TestData.INVALID_EMAIL, TestData.VALID_PASSWORD);
        when(authManager.authenticate(token)).thenReturn(token);
        when(userRepo.findByEmail(TestData.INVALID_EMAIL)).thenReturn(Optional.empty());

        assertThrows(UserNotFoundException.class,
                () -> authService.authenticate(TestData.INVALID_LOGIN));
    }
    ...
    void register_shouldThrowIfEmailAlreadyExists() {
        ...
        assertThrows(EmailExistsException.class,
                () -> authService.register(TestData.REGISTER_REQUEST));
    }
    ...
    void login_shouldThrowIfUserNotFound() {
        ...
        assertThrows(IllegalArgumentException.class,
                () -> authService.login(TestData.VALID_LOGIN));
    }
