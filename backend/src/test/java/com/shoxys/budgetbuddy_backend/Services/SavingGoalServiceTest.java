...
class SavingGoalServiceTest {
    ...
    private final Account goalAccount = new Account("Goals", AccountType.GOALSAVINGS, null, BigDecimal.TEN, true, mockUser);
    private final SavingGoal savingGoal = new SavingGoal("Save for Car", BigDecimal.valueOf(1000), BigDecimal.valueOf(200), LocalDate.now(), "img.jpg", goalAccount, mockUser);
    private final SavingGoal savingGoal2 = new SavingGoal("Vacation", BigDecimal.valueOf(100), BigDecimal.valueOf(20), LocalDate.now(), "img.jpg", goalAccount, mockUser);
    ...
    void getSavingGoalTitleById_shouldReturnTitle() {
        ...
        when(savingGoalsRepo.findTitleForSavingGoalByIdAndUser(GOAL_ID, mockUser)).thenReturn("Save for Car");
    ...
    void createSavingGoal_shouldCreateAndReturnGoal() {
        SavingGoalRequest req = new SavingGoalRequest("Title", BigDecimal.valueOf(1000), BigDecimal.valueOf(0), LocalDate.now(), "img");
        when(userRepo.getUserByEmail(VALID_EMAIL)).thenReturn(Optional.of(mockUser));
        when(accountRepo.findAccountByUserAndType(mockUser, AccountType.GOALSAVINGS)).thenReturn(Optional.empty());
        when(accountService.createGoalSavingsAccount(mockUser, req.getContributed())).thenReturn(goalAccount);
    ...
    void updateSavingGoal_shouldModifyAndReturnGoal() {
        SavingGoalRequest req = new SavingGoalRequest("Updated Title", BigDecimal.valueOf(800), BigDecimal.valueOf(200), LocalDate.now(), "newimg.jpg");

        when(userRepo.getUserByEmail(VALID_EMAIL)).thenReturn(Optional.of(mockUser));
        when(savingGoalsRepo.findSavingGoalByIdAndUser(GOAL_ID, mockUser)).thenReturn(Optional.of(savingGoal));
        when(savingGoalsRepo.save(any(SavingGoal.class))).thenAnswer(inv -> inv.getArgument(0));
    ...
    void getPendingSavingGoalsForUser_shouldReturnSavingGoals() {
        List<SavingGoal> expectedPendingGoals = Arrays.asList(
                savingGoal,
                savingGoal2
        );
        when(userRepo.getUserByEmail(VALID_EMAIL)).thenReturn(Optional.of(mockUser));
        ...
        List<SavingGoal> actualPendingGoals = savingGoalService.getPendingSavingGoalsForUser(VALID_EMAIL);


        assertEquals(expectedPendingGoals.size(), actualPendingGoals.size());
        TestUtils.assertListElementsMatch(expectedPendingGoals, actualPendingGoals, (expected, actual) -> {
    ...
    void getCompleteSavingGoalsForUser_shouldReturnSavingGoals() {
        List<SavingGoal> expectedCompleteGoals = Arrays.asList(
                savingGoal,
                savingGoal2
        );
        when(userRepo.getUserByEmail(VALID_EMAIL)).thenReturn(Optional.of(mockUser));
        when(savingGoalsRepo.findCompletedSavingGoalsForUser(mockUser)).thenReturn(expectedCompleteGoals);

        List<SavingGoal> actualCompleteGoals = savingGoalService.getCompleteSavingGoalsForUser(VALID_EMAIL);


        assertEquals(expectedCompleteGoals.size(), actualCompleteGoals.size());
        TestUtils.assertListElementsMatch(expectedCompleteGoals, actualCompleteGoals, (expected, actual) -> {
                    assertEquals(expected.getTitle(), actual.getTitle());
                    assertEquals(expected.getDate(), actual.getDate());
                    assertEquals(expected.getContributed(), actual.getContributed());
                    assertEquals(expected.getTarget(), actual.getTarget());
    ...
    void getGoalStatsForUser_shouldReturnGoalStatResponse() {
        ...
        // Assert
        List<GoalStat> expectedStats = List.of(
        ...
        TestUtils.assertListElementsMatch(expectedStats, actual.getGoalStats(), (expected, actualStat) -> {
    ...
    void getGoalStatsForUser_shouldThrowIfUserNotFound() {
        ...
        assertThrows(UserNotFoundException.class, () ->
                savingGoalService.getGoalStatsForUser(INVALID_EMAIL)) ;
