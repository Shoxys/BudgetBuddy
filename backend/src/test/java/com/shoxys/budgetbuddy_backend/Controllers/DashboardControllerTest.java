...
class DashboardControllerTest {
  ...
  void getTotalBalance_ShouldReturnTotalBalance() throws Exception {
    ...
    mockMvc.perform(get("/api/dashboard/total-balance")
  ...
  void getAccountsSummary_ShouldReturnAccountSummaries() throws Exception {
    // Mocking the service layer
    List<AccountSummary> expectedSummary = Collections.singletonList(new AccountSummary(1, AccountType.SPENDING,"Saving Account", new BigDecimal("10000.00")));
    when(userService.getUserByEmail(anyString())).thenReturn(testUser);
    when(dashboardService.getAccountSummary(anyLong())).thenReturn(expectedSummary);

    // Performing the request and asserting the response
    mockMvc.perform(get("/api/dashboard/accounts-summary")
                    .with(user(testUserDetails)))
            .andExpect(status().isOk())
  ...
  void getNetworth_ShouldReturnNetworthResponse() throws Exception {
    // Mocking the service layer
    NetworthResponse expectedNetworth = new NetworthResponse(new BigDecimal("50000.00"), Collections.nCopies(5, new BreakdownItem("Savings", BigDecimal.valueOf(10000))));
    when(userService.getUserByEmail(anyString())).thenReturn(testUser);
    when(dashboardService.getNetworthResponse(anyLong())).thenReturn(expectedNetworth);

    // Performing the request and asserting the response
    mockMvc.perform(get("/api/dashboard/networth")
                    .with(user(testUserDetails)))
            .andExpect(status().isOk())
  ...
  void getSpendingInsights_ShouldReturnSpendingInsights() throws Exception {
    // Mocking the service layer
    List<SpendingInsight> expectedInsights = Collections.singletonList(new SpendingInsight("Cut down on eating out"));
    when(userService.getUserByEmail(anyString())).thenReturn(testUser);
    when(dashboardService.getSpendingInsights(anyLong())).thenReturn(expectedInsights);

    // Performing the request and asserting the response
    mockMvc.perform(get("/api/dashboard/spending-insights")
                    .with(user(testUserDetails)))
            .andExpect(status().isOk())
  ...
  void getSavingGoals_ShouldReturnSavingGoalSummaries() throws Exception {
    // Mocking the service layer
    List<SavingGoalSummary> expectedGoals = Collections.singletonList(new SavingGoalSummary("Vacation", new BigDecimal("2000.00"), new BigDecimal("500.00")));
    when(userService.getUserByEmail(anyString())).thenReturn(testUser);
    when(dashboardService.getSavingGoalSummary(anyLong())).thenReturn(expectedGoals);

    // Performing the request and asserting the response
    mockMvc.perform(get("/api/dashboard/saving-goals")
                    .with(user(testUserDetails)))
            .andExpect(status().isOk())
  ...
  void getIncomeExpenseSummary_ShouldReturnIncomeExpenseSummaries() throws Exception {
    // Mocking the service layer
    List<IncomeExpenseSummary> expectedSummary = Collections.singletonList(new IncomeExpenseSummary("July", new BigDecimal("3000.00"), new BigDecimal("1500.00")));
    when(userService.getUserByEmail(anyString())).thenReturn(testUser);
    when(dashboardService.getIncomeExpenseSummary(anyLong())).thenReturn(expectedSummary);

    // Performing the request and asserting the response
    mockMvc.perform(get("/api/dashboard/income-expense-summary")
                    .with(user(testUserDetails)))
            .andExpect(status().isOk())
            .andExpect(content().json(objectMapper.writeValueAsString(expectedSummary)));
  }
  ...
  void getIncomeTrend_ShouldReturnIncomeTrend() throws Exception {
    // Mocking the service layer
    IncomeTrend expectedTrend = new IncomeTrend(Collections.singletonList("Jan"), Collections.singletonList(new BigDecimal("3000")), Collections.singletonList(new BigDecimal("3000")));
    when(userService.getUserByEmail(anyString())).thenReturn(testUser);
    when(dashboardService.getIncomeTrend(anyLong())).thenReturn(expectedTrend);

    // Performing the request and asserting the response
    mockMvc.perform(get("/api/dashboard/income-trend")
                    .with(user(testUserDetails)))
            .andExpect(status().isOk())
  ...
  void getExpenseAnalysis_ShouldReturnExpenseAnalyses() throws Exception {
    // Mocking the service layer
    List<ExpenseAnalysis> expectedAnalysis = Collections.singletonList(new ExpenseAnalysis("Food", new BigDecimal("400.00")));
    when(userService.getUserByEmail(anyString())).thenReturn(testUser);
    when(dashboardService.getExpenseAnalysis(anyLong())).thenReturn(expectedAnalysis);

    // Performing the request and asserting the response
    mockMvc.perform(get("/api/dashboard/expense-analysis")
                    .with(user(testUserDetails)))
            .andExpect(status().isOk())
  ...
  void getRecentTransactions_ShouldReturnRecentTransactions() throws Exception {
    // Mocking the service layer
    List<RecentTransactions> expectedTransactions = Collections.singletonList(new RecentTransactions(new Date(2025,05,01), "Starbucks", "Shopping",  new BigDecimal("-5.75")));
    when(userService.getUserByEmail(anyString())).thenReturn(testUser);
