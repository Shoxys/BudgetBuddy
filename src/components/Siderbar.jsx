    ...
    return (
     <nav className={`fixed top-0 left-0 bottom-0 z-50 flex flex-col px-3 py-1 gap-2 bg-white shadow-bb-general transition-[width] duration-200 ${expanded ? 'w-80 px-5 opacity-90' : 'w-16 px-2 items-center'}`}>
        <div className="flex flex-row">
            ...
            <button
                onClick={setNavState}
                className={`absolute top-2 right-[-14px] w-5 h-10 opacity-100 bg-white rounded-md border border-gray-400 shadow-bb-general hover:bg-slate-200 outline-none flex items-center justify-center`}
                >
                <img
